...
def get_junctions(jloc, joff, topk = 300, th=0):
    # jloc: torch.Size([1, 128, 128])
    # joff: torch.Size([2, 128, 128])
    height, width = jloc.size(1), jloc.size(2)
    jloc = jloc.reshape(-1)  # torch.Size([128x128])
    joff = joff.reshape(2, -1)  # torch.Size([2, 128x128])

    scores, index = torch.topk(jloc, k=topk)  # index (among 128x128)
...
def get_pred_junctions(convex_map, concave_map, joff_map):
    # convex_map: torch.Size([1, 128, 128]), convex vertex segmentation probability map
    # concave_map: torch.Size([1, 128, 128]), concave vertex segmentation probability map
    # joff_map: torch.Size([2, 128, 128])
    # concave junctions
    concave_pred_nms = non_maximum_suppression(concave_map)  # torch.Size([1, 128, 128])
    topK = min(300, int((concave_pred_nms>0.008).float().sum().item()))  # 300
...
def generate_polygon(prop, mask, juncs, pid, test_inria, file_name):
    # process per building
    if not test_inria:
        poly, score, juncs_pred_index = get_poly_crowdai(prop, mask, juncs, file_name)
...
def get_poly_crowdai(prop, mask_pred, junctions, file_name):
    # process per building
