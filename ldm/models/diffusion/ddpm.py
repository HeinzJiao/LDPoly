    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=True):  # modified, only load unet
        ...
        if not only_model:  # False
            ...
            for pk, k in zip(sorted(pretrain_sd.keys()), sorted(sd.keys())):
                # e.g.
                # pretrain_sd[pk]:  input_blocks.0.0.weight, torch.Size([192, 4, 3, 3])
                # sd[k]:  input_blocks.0.0.weight, torch.Size([192, 8, 3, 3])
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        """Restored Conditioning-model from models/first_stage_models/kl-f8/model.ckpt with 0 missing and 0 unexpected 
        keys"""
        # self.instantiate_cond_stage(cond_stage_config)
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        x = super().get_input(batch, k)  # 4x3x256x256, (binary) range -1 -> 1
        ...
        z = self.get_first_stage_encoding(encoder_posterior).detach()  # 4x4x32x32 *self.scale_factor
        ...
        out = [z, c, x, cls_id]  # 4x4x32x32, 4x3x256x256, 4x3x256x256, [-1, -1, -1, -1]
    ...
    def forward(self, x, c, cls_id, *args, **kwargs):
        # x: 4x4x32x32 c: 4x3x256x256 cls_id: tensor([-1, -1, -1, -1]) seg_label: 4x3x256x256 binary -1 to 1
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
    ...
    def p_losses(self, x_start, cond, t, seg_label, noise=None):  #
        ...
        # c: dict(c_concat=[4x4x32x32], c_crossattn=[cls_id]), latent image
        # seg_label: bx3x256x256, binary -1 to 1
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy, t, cond)  # bx4x32x32
        ...
        def get_dice(data, used_sampler="ddim", save_dir=None, ddim_steps=50):
            ...
            def get_dice_loop(data, sampler, use_direct=False, noise=None, save_dir=None, ddim_steps=50):
                ...
                for prompts in pbar:
                    ...
                    if image.shape[-1] > 3:     # for gray-scale 3D dataset inference, not used
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=True, ddim_eta=1., return_keys=None,
        ...
        N = min(x.shape[0], N)
        n_row = min(x.shape[0], n_row)
        log["inputs"] = x  # input segmentation mask
        # Use make_grid to merge images into a grid, displaying z_dim images per row.
        log["latent"] = self.prepare_latent_to_log(z)  # 4x4x32x32 -> 16x1x32x32 -> 3x138x138
