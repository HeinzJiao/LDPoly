from ldm.models.diffusion.ddpm_vaihingen_map_generalization import LatentDiffusion
import torch
import tqdm
...
class ExtendedLatentDiffusion(LatentDiffusion):
    ...
    def log_images_loop(self, data, save_results, save_dir, used_sampler='ddpm', save_samples_seg_logits_npy=True, save_samples_heat_logits_npy=True):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            x_out = (x_out_p > 0.5)
            # x_out: reconstructed segmentation mask
            x_prediction = x_out[:, :, 0]  # HxW
    ...
    def log_images(self, batch, N=8, n_row=4,
        ...
        if return_first_stage_outputs:
            ...
            z, zh, c_mask, c_heatmap, x, h, cls_id = self.get_input(batch, self.first_stage_key,
                                                                    return_first_stage_outputs=False,
                                                                    force_c_encode=True,
        ...
        if sampler == 'direct':
            with self.ema_scope():
                ...
                model_output = self.apply_model(noise_x, noise_h, final_t, c['c_concat'][0], c['c_concat'][1])
            _, d, _, _ = model_output.shape
            split_size = d // 2
            samples = self.predict_start_from_noise(noise_x, final_t, noise=model_output[:, :split_size, :, :])  # latent x0
            samples_h = self.predict_start_from_noise(noise_h, final_t, noise=model_output[:, split_size:, :, :])  # latent h0
