...
from ldm.models.diffusion.custom_ddim_vaihingen_map_generalization import DDIMSampler, DDIMSamplerWithGrad
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # 用于设置模型保存的监控指标（如 "val_avg_dice"），
        # 将传递给 main.py 种定义的 ModelCheckpoint 回调函数以决定保存策略：
        # - 若指定 monitor，则按该指标保存性能最优的模型；
        # - 若未指定，则按固定周期（如每 10 个 epoch）保存。
        if monitor is not None:
            self.monitor = monitor

        # 如果 config.yaml 中提供了 ckpt_path（例如 "models/ldm/lsun_churches256/model.ckpt"），
        # 就加载 Stable Diffusion 中预训练好的 UNet 权重，用于 finetune；
        # 否则表示从头训练（train from scratch）。
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=True):
        """
        加载模型权重的辅助函数，用于初始化 LatentDiffusion 模型。
        - 训练阶段：仅加载 UNet（Diffusion Model）。
        - 推理阶段：加载完整模型（包括 first-stage autoencoder, cond-stage, diffusion model）。

        Args:
            path (str): checkpoint 路径。
            ignore_keys (list): 要从 checkpoint 中忽略的 key 前缀。
            only_model (bool): 是否只加载 UNet。训练时设为 True。
        """

        # 当前模型的 diffusion model（UNet）状态字典
        ...
        if not only_model:
            # 推理阶段，加载整个模型（包括 cond-stage 和 first-stage）
            sd = torch.load(path, map_location="cpu")
            if "state_dict" in list(sd.keys()):
                sd = sd["state_dict"]

            # 移除需要忽略的 key（例如 optimizer 参数等）
            ...
            # 加载整个 LightningModule 的权重（非 strict）
            ...
            if "label_emb" in self.unet_sd_keys:  # False
                label_emb_keys = [key for key in sd.keys() if "label_emb" in key]
                label_emb_tmp = [sd.pop(key) for key in label_emb_keys]
            else:
                label_emb_tmp = None

            # 清除 checkpoint 中非 diffusion_model 的参数，仅保留 model.diffusion_model. 前缀的 key
            for k in list(pretrain_sd.keys()):
                if not k.startswith("model."):
                    del pretrain_sd[k]
                else:
                    v = pretrain_sd.pop(k)
                    # 只保留 diffusion_model 的子模块（如 input_blocks）
                    new_k = k.replace("model.diffusion_model.", "")
                    pretrain_sd[new_k] = v

            # 对比每个参数 key 和 shape，如果不匹配，则填充 0 或适配
    ...
    def training_step(self, batch, batch_idx):
        """
        pl.LightningModule的固定函数
        用途：定义每个训练步骤的操作，必须返回 loss。
        返回值：通常返回一个 torch.Tensor（loss），也可以返回一个 dict。
        """
        ...
        # 将其余的 loss 指标记录到日志系统，可视化训练情况（如 wandb, tensorboard）
        self.log_dict(
            loss_dict,
            prog_bar=True,  # 显示在进度条上
            logger=True,  # 记录到日志
            on_step=True,  # 每 step 都记录
            on_epoch=False  # 每 epoch 不聚合
        )

        # 记录当前 global step，用于日志追踪
        ...
        print("Current Learning Rate: ", lr)  # 输出当前学习率到控制台

        # ✅（可选）记录模型第一层卷积核的初始化状态：
        # input_blocks.0.0.weight 的 shape 通常是 [out_channels, in_channels, H, W]
        # 下面这段代码监测权重前 4 个通道（假设是来自 pretrained image）和后 4 个通道（假设是 zero-init extra input）的平均值。
        # 这通常用于分析模型的融合策略是否有效，比如 image 和 mask 被输入到模型时是否学到了不同的特征。
        # if batch_idx % 10 == 0:
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
                 ...
                 *args, **kwargs):
        """
        LatentDiffusion 初始化函数，参数通常由 config['model'] 自动传入。

        Args:
            first_stage_config: autoencoder 配置（如用于 mask 编码）。
            cond_stage_config: 条件 autoencoder 配置（如用于 image 编码）。
            num_timesteps_cond: 条件时间步数量（一般为 1）。
            cond_stage_key: 从 batch 中提取条件的 key。
            num_classes: 分类任务类别数（未启用时无影响）。
            cond_stage_trainable: 条件编码器是否可训练。
            concat_mode: 是否使用 concat 模式进行条件融合。
            cond_stage_forward: 自定义条件前向函数（可选）。
            conditioning_key: 条件类型（如 "concat" 或 "crossattn"）。
            scale_factor: 对 latent 进行缩放的因子。
            scale_by_std: 是否按标准差 scale。
            load_only_unet: 是否仅加载 UNet 权重。
        """
        ...
        # 从 config 中提取 checkpoint 路径与需要忽略的 key
        ckpt_path = kwargs.pop("ckpt_path", None)
        ignore_keys = kwargs.pop("ignore_keys", [])

        # 初始化 DDPM 基类（包括 denoiser 等组件）
        ...
        # 获取 autoencoder 的下采样层数（便于生成 positional encoding）
        try:
        ...
        if not scale_by_std:
            ...
            for key in self.first_stage_key:
                setattr(self, f'scale_factor_{key}', scale_factor)
        else:
            self.register_buffer('scale_factor', torch.tensor(scale_factor))
            for key in self.first_stage_key:
                self.register_buffer(f'scale_factor_{key}', torch.tensor(scale_factor))

        # 初始化第一阶段 autoencoder（如 mask / heatmap 编码器）
        self.instantiate_first_stage(first_stage_config)

        # 初始化条件编码模块（如 image 编码器）
        self.instantiate_cond_stage(cond_stage_config)

        if ckpt_path is not None:  # 加载预训练的 UNet
            self.init_from_ckpt(ckpt_path, ignore_keys, only_model=self.load_only_unet)
            self.restarted_from_ckpt = True
        else:
            # 若从头训练 UNet，保存 UNet 的 key 集合（用于后续加载对比）
            sd = self.model.diffusion_model.state_dict()
            self.unet_sd_keys = set(map(lambda x: x.split(".")[0], sd.keys()))
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        """
        Args:
            batch: 一个 dict，包含当前 batch 的所有输入字段
            k: list[str]，表示使用哪些输入作为第一阶段模型的输入（例如 ['segmentation', 'heat']）
            return_first_stage_outputs: 是否返回解码后的重建图（如 xrec）
            force_c_encode: 是否强制对条件进行编码（用于控制是否使用 self.get_learned_conditioning）
            cond_key: 指定条件输入的 key（如 'image'）；默认从 self.cond_stage_key 中获取
            return_original_cond: 是否返回原始条件 xc（如用于 log）
            bs: 限制 batch size，用于 debug 或 inference 时手动减小 batch

        Returns:
            list: 包含 latent 表征、条件输入、原始图像、class_id 以及（可选）解码图像和原始条件
        """
        x_list = []  # 原始输入图像（x 和 h）
        z_list = []  # 编码后的 latent 表征（zx 和 zh）
        cls_id = batch["class_id"][:, 0]

        # ==== 编码 segmentation 和 heatmap 为 latent 表征 ====
        for i in range(len(k)):
            # 获取第 i 个输入（segmentation 或 heatmap），范围 [-1, 1]，尺寸 [B, 3, H, W]
            x = super().get_input(batch, k[i])
            if bs is not None:
                x = x[:bs]
            x = x.to(self.device)
            x_list.append(x)  # torch.Size([B, 3, H, W])

            # 编码为 latent（通常为 4x32x32），返回的是 Gaussian 分布
            encoder_posterior = self.encode_first_stage(x)
            z = self.get_first_stage_encoding(encoder_posterior, ztype=k[i]).detach()
            z_list.append(z)  # torch.Size([B, 4, H/8, W/8])

        # ==== 构造条件输入 ====
        if self.model.conditioning_key is not None:  # e.g., 'concat'
            # 当前使用两个固定条件：raw_mask 和 raw_heatmap
            xc_mask = super().get_input(batch, 'raw_mask').to(self.device)  # [B, 3, H, W], range -1 -> 1
            xc_heatmap = super().get_input(batch, 'raw_heatmap').to(self.device)  # [B, 3, H, W], range -1 -> 1

            if not self.cond_stage_trainable or force_c_encode:  # False during training / True during log_images
                if isinstance(xc_mask, dict) or isinstance(xc_mask, list):
                    c_mask = self.get_learned_conditioning(xc_mask)
                    c_heatmap = self.get_learned_conditioning(xc_heatmap)
                else:
                    c_mask = self.get_learned_conditioning(xc_mask.to(self.device))
                    c_heatmap = self.get_learned_conditioning(xc_heatmap.to(self.device))
            else:  # True
                c_mask = xc_mask
                c_heatmap = xc_heatmap

        else:  # False
            c_mask, c_heatmap = None, None
            xc_mask, xc_heatmap = None, None

        # ==== 构造输出 ====
        out = z_list + [c_mask, c_heatmap] + x_list + [cls_id]
        # out: zx, zh, c_mask, c_heatmap, x, h, cls_id
        # 4x4x16x16, 4x3x16x16, 4x3x128x128, 4x3x128x128, 4x3x128x128, 4x3x128x128, [-1, -1, -1, -1]
        ...
        if return_first_stage_outputs:  # False during training / True during log_images
            for i in range(len(z_list)):
                ...
                out.append(xrec)
        # out: zx, zh, c_mask, c_heatmap, x, h, cls_id, xrec, hrec

        if return_original_cond:  # False during training / True during log_images
            out.append(xc_mask)
            out.append(xc_heatmap)
    ...
    def shared_step(self, batch, **kwargs):
        x, h, cond_mask, cond_heatmap, seg_label, vert_heatmap, cls_id = self.get_input(batch, self.first_stage_key)
        # zx, zh, c1, c2, x, h, cls_id
        # c: 4x3x256x256 condition image, range -1 to 1
        loss = self(x, h, cond_mask, cond_heatmap, cls_id, seg_label)
        return loss

    def forward(self, x, h, cond_mask, cond_heatmap, cls_id, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:  # self.model.conditioning_key = concat
            if self.cond_stage_trainable:  # True
                cond_mask = self.get_learned_conditioning(cond_mask)  # 4x4x32x32
                cond_heatmap = self.get_learned_conditioning(cond_heatmap)  # 4x4x32x32
        ...
        loss, loss_dict = self.p_losses(x, h, cond_mask, cond_heatmap, t, *args, **kwargs)
        return loss, loss_dict
    ...
    def apply_model(self, x_noisy, h_noisy, t, cond_mask, cond_heatmap, return_ids=False):
        # cond: c, torch.Size([4, 4, 32, 32])
        cond = [cond_mask, cond_heatmap]
    ...
    def p_losses(self, x_start, h_start, cond_mask, cond_heatmap, t, seg_label, noise=None):  #
        ...
        # run the denoiser
        model_output = self.apply_model(x_noisy, h_noisy, t, cond_mask, cond_heatmap)
    ...
    def p_mean_variance(self, x, h, c, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        # c={"c_concat": [cond_mask (4x4x16x16), cond_heatmap (4x4x16x16)]}
        # t: e.g. tensor([999, 999, 999, 999])
        t_in = t
        model_out = self.apply_model(x, h, t_in, c['c_concat'][0], c['c_concat'][1], return_ids=return_codebook_ids)
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        ...
        if ddim:  # True
            ...
            # e.g., intermediates={'x_inter': [gaussian noise(4x32x32), ... ]
    ...
    def log_images(self, batch, N=8, n_row=4,
                   sample=True,
                   ddim_steps=True, ddim_eta=1., return_keys=None,
        ...
        z, zh, c_mask, c_heatmap, x, h, cls_id, xrec, hrec, xc_mask, xc_heatmap = self.get_input(batch,
                                                                                                 self.first_stage_key,
                                                                                                 return_first_stage_outputs=True,
                                                                                                 force_c_encode=True,
                                                                                                 return_original_cond=True,
                                                                                                 bs=N)
        # z, zh,                   # torch.Size([4, 4, 16, 16]) -> latent vector of building mask & vertex heatmap
        # c_mask, c_heatmap,       # torch.Size([4, 4, 16, 16]) -> condition 的 latent vector（由 raw_mask/raw_heatmap 编码而来）
        # x, h,                    # torch.Size([4, 3, 128, 128]) -> 原始 pixel-space mask 和 heatmap（已归一化 & 扩展成3通道）
        # cls_id,                  # torch.Size([4])              -> 每张图像对应的类别 ID（如 -1，未使用时常为占位）
        # xrec, hrec,              # torch.Size([4, 3, 128, 128]) -> 由 autoencoder 解码 z 和 zh 得到的重建图像（mask & heatmap）
        # xc_mask, xc_heatmap      # torch.Size([4, 3, 128, 128]) -> raw mask 和 raw heatmap condition 的原始图（pixel space）

        print(f"[logging class ID]: {cls_id.detach().cpu()}")
        if self.model.conditioning_key == 'concat':
            c = {'c_concat': [c_mask, c_heatmap]}
        elif self.model.conditioning_key == 'crossattn':
            c = {'c_crossattn': [c_mask, c_heatmap]}
        else:  # hybrid
            c = {'c_concat': [c_mask. c_heamtap], 'c_crossattn': [c_mask, c_heatmap]}
        ...
        log["latent_seg"] = self.prepare_latent_to_log(z)  # e.g., 4x4x32x32 -> 16x1x32x32 -> 3x138x138
        ...
        if self.model.conditioning_key is not None:  # concat
            log["conditioning_mask"] = xc_mask
            log["conditioning_heatmap"] = xc_heatmap
            if self.conditioning_key == 'concat':
                log["conditioning_latent_mask"] = self.prepare_latent_to_log(c['c_concat'][0])
                log["conditioning_latent_heatmap"] = self.prepare_latent_to_log(c['c_concat'][1])
            else:
                log["conditioning_latent_mask"] = self.prepare_latent_to_log(c['c_crossattn'][0])
                log["conditioning_latent_heatmap"] = self.prepare_latent_to_log(c['c_crossattn'][1])
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, h, t, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            ...
            xc = torch.cat([x] + [h] + c_concat, dim=1)  # 4, 16, 32, 32
            out = self.diffusion_model(xc, t)  # 4, 8, 32, 32
        # NotImplementedError
        # elif self.conditioning_key == 'crossattn':
        #     xh = torch.cat([x] + [h], dim=1)  # torch.Size([4, 8, 32, 32])
        #     cc = torch.cat(c_crossattn, 1)  # torch.Size([4, 4, 32, 32])
