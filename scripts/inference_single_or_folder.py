...
@torch.no_grad()
def inference(model, image, save_dir, image_name, sampler='ddim', device='cuda'):
    # 预处理图像 batch，模拟 dataloader 的 batch 结构
    batch = {
        'image': image.to(device),  # 1, H, W, 3, range -1 to 1
        'segmentation': torch.zeros_like(image[:, :, :, :]),  # dummy segmentation
        'heatmap': torch.zeros_like(image[:, :, :, :]),  # dummy heatmap
        'file_path_': [image_name],
        'class_id': torch.tensor([[-1]]).to(device)  # 修复这里！
    }
...
def main():
    ...
    parser.add_argument('--input', type=str, required=True, help='Path to input image or folder of images')
    ...
    if os.path.isfile(opt.input):
        # 单张图片
        image = load_image(opt.input)
        image_name = os.path.splitext(os.path.basename(opt.input))[0]
        seg_npy, heat_npy = inference(model, image, opt.outdir, image_name, sampler=opt.sampler, device=device)
        ...
        image = image.squeeze(0).cpu().numpy()  # H, W, 3, range: [-1, 1]
        image = ((image + 1.0) / 2.0 * 255.0).astype(np.uint8)  # H, W, 3, range: [0, 255]
        image = image[..., ::-1]  # RGB to BGR for OpenCV
        visualize_predictions(image, polygons, image_name, opt.outdir)

    elif os.path.isdir(opt.input):
        # 文件夹多张图片
        image_paths = glob.glob(os.path.join(opt.input, '*'))
        for image_path in tqdm(image_paths, desc="Processing images"):
            ...
            for i, prop in enumerate(props):
                # Extract polygon for each building region
                poly_list, score = get_poly(prop, logit, junctions, d_th=5)  # poly_list: [[x1, y1, x2, y2, ...], [x1, y1, ...], ...]
                if len(poly_list) == 0:
                    continue
                polygons.append(poly_list)
            ...
            image = ((image + 1.0) / 2.0 * 255.0).astype(np.uint8)  # H, W, 3, range: [0, 255]
            image = image[..., ::-1]  # RGB to BGR for OpenCV
            os.makedirs(os.path.join(opt.outdir, "polygons_vis"), exist_ok=True)
            visualize_predictions(image, polygons, image_name, os.path.join(opt.outdir, "polygons_vis"))
    else:
...
if __name__ == '__main__':
    ...
    """
    PYTHONPATH=./:$PYTHONPATH python3 scripts/inference_single_or_folder.py --input "./data/inria/tyrol-e5" \
        --outdir "./outputs/inria/tyrol-e5" \
